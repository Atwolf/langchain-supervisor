...
- Anthropic Codex Codex-sonnet-4-5-20250514 (sub-agents and supervisor)
...
- `public/elements/QaEntrypoint.jsx` — isolated start card that invokes the QA wizard action callback
- `public/elements/QaWizard.jsx` — multi-phase custom element that collects the target architecture JSON model
- `src/qa_wizard.py` — QA wizard options, empty model factory, id generation, and normalization helpers
- `docs/qa-json-wizard.md` — detailed QA wizard contract, validation rules, and extension guidance
...
### QA JSON Wizard
The QA JSON Wizard is an action-driven Chainlit workflow for collecting architecture facts into a deterministic target JSON model. It intentionally bypasses the supervisor and sub-agent abstraction layer because the workflow is structured form collection, not an LLM-routed task.

**Entrypoint**: `chainlit_app.py` sends `public/elements/QaEntrypoint.jsx` on chat start. The entrypoint renders an isolated `Architecture QA Wizard` card and invokes the `qa_start` action callback when clicked. This replaced the slash command trigger because Chainlit slash commands require composer submission before backend handling runs, while a normal action callback starts immediately.

**Wizard Flow**: `qa_start` calls `run_qa_wizard()`, which opens `public/elements/QaWizard.jsx` through `cl.AskElementMessage`. The custom element manages phases for system details, access/edge details, components, data stores, messaging, external systems, and final review. It validates required fields before phase transitions and submits the model with `submitElement({ model })`.

**Normalization Boundary**: Submitted payloads are normalized by `src/qa_wizard.py`. The service owns option lists, the empty model factory, JSON formatting, stable id generation from names, and relationship cleanup. Component relationships store component ids rather than display labels, and stale relationship ids are filtered during normalization.

**Session Scope**: Generated JSON is stored only in `cl.user_session["qa_model"]` for this POC. `qa_restart` reopens the wizard with the last session model, and `qa_show_json` resends the latest model. There is no database persistence, file export, or model-provider call in this flow.

**Documentation**: `docs/qa-json-wizard.md` is the detailed contract for the wizard, including the JSON schema shape, required fields, id behavior, action callbacks, and extension points. Update that file whenever the target model or interaction boundary changes. No Model Decisions entry is required for wizard-only changes unless the flow starts invoking an LLM.
...
### Model Decisions
- **Supervisor LLM**: Codex-sonnet-4-5-20250514 — chosen for fast tool-calling with good routing accuracy
